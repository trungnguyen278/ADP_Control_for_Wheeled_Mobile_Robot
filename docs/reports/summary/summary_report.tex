...
\graphicspath{{../thesis/figures/}{../sm2_report/figures/}}
...
\begin{center}
    ...
    \begin{tabular}{rl}
        ...
        \textbf{Chuyên ngành:}  & Kỹ thuật Điều khiển và Tự động hóa --- Đại học Bách khoa Hà Nội \\
...
Gần đây, Wang và cộng sự (IEEE RA-L, 2025)~\cite{Wang2025} đã đề xuất phương pháp điều khiển tối ưu dựa trên Quy hoạch động thích nghi (Adaptive Dynamic Programming -- ADP) với tính chất hội tụ cố định thời gian (fixed-time convergence). Phương pháp này đảm bảo trọng số mạng nơ-ron Critic hội tụ về giá trị tối ưu trong thời gian hữu hạn, không phụ thuộc vào điều kiện đầu. Tuy nhiên, công trình gốc chỉ xét mô hình \textbf{kinematic} đơn giản, bỏ qua hoàn toàn động lực học (dynamic) của robot.
...
\begin{enumerate}
    ...
    \item \textbf{Điều chỉnh tham số robust cho dual-loop:} Các hệ số robust (fractional power, cubic terms) trong Wang et al. được thiết kế cho kinematic-only. Khi kết hợp với vòng trong đã reject nhiễu, các hệ số này cần giảm 3--5 lần để tránh phát tán. Luận văn trình bày quy trình điều chỉnh chi tiết và phân tích nguyên nhân.
...
\begin{itemize}
    ...
    \item \textbf{SMC:} Hiệu năng kém nhất trên mọi quỹ đạo ($J_c = 555.5$ trên đường tròn, $J_c = 2457.6$ trên hình số 8). SMC cơ bản tạo ra chattering lớn, truyền qua vòng trong gây dao động mô-men liên tục. Kết luận: \textbf{SMC không phù hợp} với kiến trúc dual-loop kiểu này.
\end{itemize}
...
Khảo sát ảnh hưởng của biên độ nhiễu ngoài $d(t) = A \cdot \tau_\text{max} \cdot \sin(30t)$ với $A \in \{0, 0.2, 0.4, 0.6\}$ (tương ứng 0\%, 20\%, 40\%, 60\% mô-men cực đại). Quỹ đạo: đường tròn, $m = 10$\,kg.
...
\midrule
$\lambda_\text{ft}$ & 0.30 & 0.08 & Hệ số fractional power giảm 3.75$\times$ \\
$\mu_\text{ft}$     & 0.30 & 0.08 & Tương tự $\lambda$ \\
$\alpha_\text{ft}$   & 0.30 & 0.08 & Tương tự $\lambda$ \\
$\beta_\text{ft}$    & 1.00 & 0.15 & Hệ số cubic term giảm 6.7$\times$ \\
$\rho_\text{ft}$     & 0.05 & 0.10 & Tăng 2$\times$ (ổn định hơn) \\
$\Gamma_\text{ft}$   & $2I$  & $1I$  & Tốc độ học giảm 2$\times$, tránh dao động trọng số \\
$\kappa_1$           & 0.02 & 0.04 & Regularization mạnh hơn 2$\times$ \\
...
Do đó, vòng ngoài chỉ cần đối phó với sai lệch mô hình còn lại (residual), các hệ số robust cần giảm 3--5 lần. Ngoài ra, \textbf{clamp $u_o^\text{max} = 1.5$} là cơ chế an toàn cốt lõi: giới hạn biên độ tín hiệu robust, tránh trường hợp mạng Critic chưa hội tụ tạo ra lệnh quá lớn gây mất ổn định. Cơ chế này tương tự $u_o^\text{max}$ đã được sử dụng thành công trong bộ điều khiển CL.
...
\bibitem{Wang2025}
Y.~Wang, Z.~Li, C.~Yang, ``ADP-based fixed-time optimal tracking control for wheeled mobile robots,''
\textit{IEEE Robotics and Automation Letters (RA-L)}, vol.~10, no.~1, pp.~460--467, 2025.
